...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseMotion: # If the mouse moved...
		var target = event.position - get_viewport().size * 0.5	# Get the mouse position relative to the middle of the screen
		if target.length() < DEAD_ZONE:	# If we're in the middle (dead zone)...
